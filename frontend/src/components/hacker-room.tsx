...
function makeScreenTexture() {
  ...
  ctx.fillText("~ ÔÇö farley@dev: zsh", 50, 14);
  // Code content
  const lines = [
    { prompt: "$", text: "npm run test:e2e --headed", color: "#22d3ee" },
    { prompt: ">", text: "Running 420 tests...", color: "#94a3b8" },
    { prompt: "Ô£ô", text: "auth.spec.ts (42)", color: "#22c55e" },
    { prompt: "Ô£ô", text: "dashboard.spec.ts (58)", color: "#22c55e" },
    { prompt: "Ô£ô", text: "payments.spec.ts (36)", color: "#22c55e" },
    { prompt: "Ô£ô", text: "api-contracts.spec.ts (61)", color: "#22c55e" },
...
function makeLaptopScreenTexture() {
  ...
  ctx.fillText("farley-portfolio ÔÇö VS Code", 4, 10);
  // Code lines
  const codeLines = [
...
function makePictureTexture() {
  ...
  ctx.fillText("// focus ┬À build ┬À repeat", 26, 370);
...
export function HackerRoom() {
  ...
  useEffect(() => {
    ...
    // Back wall ÔÇö split into 4 pieces around window cutout
    ...
    // Sky (gradient plane) ÔÇö color changes with theme
    ...
    // Desk LED strip ÔÇö thin bar under front edge
    ...
    // Bias lighting behind monitor ÔÇö rim glow plane
    ...
    // Shelf y=2.5 top=2.525, cube half-height=0.11 ÔåÆ center at 2.635
    ...
    // Shelf y=1.8 top=1.825; pot height 0.18 ÔåÆ center at 1.915 so bottom sits on shelf
    ...
    const animate = () => {
      ...
      if (!prefersReduced) {
        ...
        // Clock hands ÔÇö normal tick + fast spin during reveal
